...
class NotificationProvider:
    ...
    # FIX BUG-36: Decorated with @abstractmethod so subclasses that forget
    # to implement send() cause an error at class-definition time, not at
    # the first call site.
    @abstractmethod
            ...
            message = {
                "attachments": [{
                    ...
                    "fields": [
                        ...
                        {"title": "Source IPs", "value": ", ".join(alert.source_ips[:5]) or "N/A"},
                        {"title": "Target IPs", "value": ", ".join(alert.target_ips[:5]) or "N/A"},
    ...
    async def send(self, alert: Alert) -> bool:
        try:
            ...
            event = {
                ...
                "links": [{"href": self.config.get('dashboard_url', ''), "text": "View Dashboard"}],
            }
            async with aiohttp.ClientSession() as session:
    ...
    async def send(self, alert: Alert) -> bool:
        try:
            ...
            body = (
                ...
                f"Source IPs: {', '.join(alert.source_ips[:10])}\n"
                f"Target IPs: {', '.join(alert.target_ips[:10])}\n\n"
                f"Details:\n{json.dumps(alert.details, indent=2)}\n"
            )
            msg.attach(MIMEText(body, 'plain'))

            # FIX BUG-23: asyncio.get_running_loop() replaces deprecated
...
class AlertManager:
    """Central alert manager: deduplication, rate limiting, and notification routing."""
    ...
    def _init_providers(self) -> None:
        if self.config.slack_enabled and self.config.slack_webhook_url:
            self.providers.append(
                SlackProvider({'webhook_url': self.config.slack_webhook_url})
            )
        ...
        logger.info(f"Initialised {len(self.providers)} notification providers")
    ...
    def _is_duplicate(self, alert: Alert) -> bool:
        for recent in self.recent_alerts:
            if (
                recent.attack_type == alert.attack_type
    ...
    def _is_rate_limited(self) -> bool:
        """
        Check per-minute alert rate limit.

        FIX BUG-35: AlertConfig.max_alerts_per_minute was defined but never
        enforced — create_alert() never checked it. Now tracked via a deque
        of timestamps, cleaned to the last 60 seconds on each call.
        """
        now = time.time()
        cutoff = now - 60.0
        while self._rate_timestamps and self._rate_timestamps[0] < cutoff:
            self._rate_timestamps.popleft()
        return len(self._rate_timestamps) >= self.config.max_alerts_per_minute

    def _schedule_notifications(self, alert: Alert) -> None:
        """
        Schedule notification sending in the right execution context.

        FIX BUG-32: The original called asyncio.get_event_loop() which is
        deprecated in Python ≥3.10 and raises RuntimeError in ≥3.12 when
        called from a non-main thread.  Now uses get_running_loop() inside
        a try/except so it works in both async and sync contexts.
        """
        try:
            loop = asyncio.get_running_loop()
            ...
            # No running event loop in this thread — dispatch to a daemon thread.
            threading.Thread(
    ...
    def create_alert(
        ...
        # FIX BUG-33: Respect config.enabled flag.
        if not self.config.enabled:
            logger.debug("AlertManager is disabled — alert suppressed")
            return None

        # Severity filter
        if (
            self._SEVERITY_ORDER.index(severity)
            < self._SEVERITY_ORDER.index(self.config.min_severity)
        ):
            logger.debug(f"Alert filtered: severity {severity.value} below minimum")
            return None

        # FIX BUG-35: Enforce per-minute rate limit before creating the alert.
        if self._is_rate_limited():
            self.stats['alerts_rate_limited'] += 1
            logger.warning(
                f"Alert rate limit reached ({self.config.max_alerts_per_minute}/min) "
                f"— suppressing: {title}"
            )
            return None
        ...
        self._rate_timestamps.append(alert.timestamp)  # FIX BUG-35
    ...
    def resolve_all_alerts(self) -> None:
    ...
    def start(self) -> None:
        ...
        self._cleanup_thread = threading.Thread(
            target=self._cleanup_loop, daemon=True, name="alert-manager-cleanup"
        )
